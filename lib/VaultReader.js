...
class VaultReader {
  ...
  async getAlerts(opts = {}) {
    ...
    for (const { entry, fm } of latestByGroup.values()) {
      ...
      // Nothing to surface: clean run with no errors and no resolution annotation
      if (!hasError) continue;

      const detail = errVal
        || (statVal   ? `status: ${fm.status}` : '')
        || (dlStatVal ? `delivery: ${fm.delivery_status}` : '');
      // resolvedFlag=true  → keep in list but mark as pending verification (not a hard alert)
      // resolvedFlag=false → active error, needs attention
      alerts.push({
        ...entry,
        alertSource: 'heartbeat-error',
        isResolved:  resolvedFlag,
        resolvedAt:  fm.resolved_at  || null,
        resolvedBy:  fm.resolved_by  || null,

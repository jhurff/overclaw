    ...
    _connectWebSocket() {
        ...
        this.ws.on('message', async (message) => {
            ...
            if (data.type === 'event' && data.event === 'connect.challenge') {
                ...
                // Generate a more robust mock signature using crypto
                const deviceId = 'overclaw-ws-client-001';
                const mockPublicKey = crypto.createHash('sha256').update(deviceId).digest('hex');
                const signaturePayload = JSON.stringify({
                    ...
                    device: { id: deviceId, nonce: nonce, signedAt: ts }
                });
                const mockSignature = crypto.createHash('sha256').update(signaturePayload).digest('hex');

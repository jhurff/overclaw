    ...
    _connectWebSocket() {
        ...
        this.ws.on('message', async (message) => {
            ...
            if (data.type === 'event' && data.event === 'connect.challenge') {
                ...
                // const ts = data.payload.ts; // Timestamp from the challenge - not needed if device object removed

                // *** OMITTING DEVICE OBJECT ENTIRELY FOR THIS TEST ***
                // No need to generate mockPublicKey or signaturePayload if device object is omitted
                ...
                const connectPayload = {
                    ...
                    params: {
                        ...
                        caps: [],
                        commands: [],
                        permissions: {}, // Added missing permissions object
                        auth: { token: this.apiToken },
                        locale: 'en-US',
                        userAgent: 'overclaw-dashboard/0.1.0',
                        // *** DEVICE OBJECT IS OMITTED HERE ***
                    }
                };

                const jsonPayload = JSON.stringify(connectPayload);
                console.log('ClawBridge: Sending connect payload (no device object):', jsonPayload);
